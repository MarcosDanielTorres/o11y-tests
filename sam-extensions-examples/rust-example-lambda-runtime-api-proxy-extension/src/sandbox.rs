...
pub mod extension {
    ...
    pub async fn get_next() {
        ...
        println!("URI: {:?}", uri.path().to_string());
        ...
        println!("Inside get_next(), about to call send_request");
        let result = super::send_request(request).await;

        let x = match result {
            Ok(val) => {
                println!("processing ok in get_next()");
                println!("body of next_event: {:?}", val.body());
            },
            Err(e) => {
                println!("processing err in get_next()");
                println!("Error in next_event: {:?}", e);
            }
        };
    }
}
